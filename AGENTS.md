...
| `fjallkartan/OfflineRegionsStore.swift` | SQLite persistence layer (`OfflineRegions.sqlite`) backing both offline-region bookkeeping (name/createdAt/pause/completed/size, keyed by `region_id`, separate from MapLibre's own `mln_region_id`) and elevation tiles (`tiles`/`region_tiles`, ref-counted so overlapping regions can share a tile). |
...
- **Offline map regions**
  - Built on `MLNOfflinePack`/`MLNOfflineStorage` via `MLNTilePyramidOfflineRegion` for the base map (z7–z14, `TilePyramid.minZoom...maxZoom`), plus a parallel elevation-tile download — both tracked in `OfflineRegionsStore`'s SQLite database (`OfflineRegions.sqlite`), which is the source of truth for name/createdAt/pause/completed/size. A pack's own `context` is just the bare region `id` (a JSON-encoded `String`, decoded by `decodeContext(_:)`) — everything else lives in the `regions` table, keyed by that same `id` (separate from MapLibre's own `mln_region_id`, which the store also records so a pack can be found for a region and vice versa).
  - `OfflineRegionsModel.startDownload(name:rect:)` inserts the region row, links its elevation tile keys (`linkTiles`), calls `MLNOfflineStorage.addPack(for:withContext:)`, records the resulting `mln_region_id`, then `resume()`s the pack and starts the elevation download using the already-computed key list (avoiding a redundant DB query for keys `startElevationDownload` would otherwise have to re-derive).
  - Progress/completion/error come from `NotificationCenter` (`.MLNOfflinePackProgressChanged`, `.MLNOfflinePackError`); `handleProgressChanged` gates on the region's already-settled displayed `status` so a paused region's first pack resolution after a relaunch is still allowed to correct stale/zero numbers rather than being silently skipped.
  - `refresh()` rebuilds `regions` from `OfflineRegionsStore.allRegions()` plus each region's `MLNOfflinePack` (only fetched via `MLNOfflineStorage.shared.packs` when at least one region is incomplete, since MapLibre never removes finished packs from that list). A freshly reloaded `MLNOfflinePack` starts `.unknown` with all-zero `progress` until something queries it, so `refresh()` calls `pack.requestProgress()` once per pack per launch (`initialProgressRequestedIDs`) the first time an incomplete region's pack resolves — this is what lets a region that finished downloading just before a force-quit report its real, not stale-zero, size.
  - The one-time completion capture (persisting the combined pack + elevation byte total via `setSize`, then `setCompleted`) is gated on the persisted `!info.completed` flag, not on `regions` (in-memory, and empty at every launch) — checking `regions` there is a trap: an already-complete region's pack can resolve non-nil for the first time long after it actually finished (e.g. the moment starting a *second* download makes `needsPacks` true), and misreading that as "just completed" would stomp the real persisted size with that pack's current, possibly-stale/never-queried progress.
  - Elevation tiles are never part of the MapLibre style, so the pack never downloads them; `startElevationDownload(id:keys:)` bulk-fetches `fetchedKeys(regionID:)` + `regionProgress(id:)` up front (off-main-thread) to compute the still-missing keys and a baseline count/bytes, then only passes the missing keys to `ElevationService.prefetchTiles(_:onProgress:)` — checking each key's cache status individually inside that call's concurrency-6 task group would otherwise run thousands of serialized SQLite lookups back to back on a big, mostly-finished region being resumed, spiking CPU and stalling the map. The progress closure adds the baseline to live incremental counts and throttles `refresh()` to ≥0.2s apart (always firing on the final tick), since cache-hit tiles resolve near-instantly and would otherwise post hundreds of ticks/second.
